  ...
  response
    ...
    .on('end', () => {
      ...
      const isBase64Encoded = isContentTypeBinaryMimeType({
        contentType, binaryMimeTypes: server._binaryTypes
      })
      const body = bodyBuffer.toString(isBase64Encoded ? 'base64' : 'utf8')
      console.log('bodyBuffer converted to: ', body)
...
function forwardRequestToNodeServer(server, event, context, resolver) {
  try {
    ...
    if (event.body) {
      const body = getEventBody(event)
      console.log('writing body out to req object:', body)
      req.write(body)
    }
...
function proxy(server, event, context, resolutionMode, callback) {
  ...
  if (!resolutionMode) {
    const resolver = makeResolver({
      context, resolutionMode: 'CONTEXT_SUCCEED'
    })
